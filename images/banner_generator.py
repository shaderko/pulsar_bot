...
async def generate_banner(info):
    ...
    ctx = ImageDraw.Draw(mask)
    ctx.pieslice(((0, 0), (600, 600)), start=CIRCLE_OFFSET,
                 end=(360-(CIRCLE_OFFSET-90)*2)*info['progress']+CIRCLE_OFFSET, fill='white')
    level_progress.paste(level, (banner.width//2-border.width//2, 10), mask)
    ...
    ctx.text((banner.width//2-font.getsize(str(info['level']))[0]//2, 462), str(info['level']),
             font=font, fill=(255, 255, 255, 255))
    ctx.text((banner.width//2-font_name.getsize(info['username'])[0]//2, 575), info['username'],
